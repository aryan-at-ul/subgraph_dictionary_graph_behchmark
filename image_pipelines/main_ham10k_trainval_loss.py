...
def setseed(seed):
    ...
    # The following lines are optional and may affect performance
...
def parse_arguments():
    ...
    parser.add_argument('--data_dir', type=str, default='/home/melkor/projects/datasets/skincancer_mnist/skin_cancer_dataset',
                        help='Directory containing the dataset organized in train and test folders')
...
def evaluate(model, device, loader, criterion_cls, logger, mode='Validation'):
    model.eval()
    total_loss = 0
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for images, labels in tqdm(loader, desc=f'{mode}', leave=False):
            images = images.to(device)
            labels = labels.to(device)
...
def main():
    ...
    log_filename = 'ham10k_gnn_trainval_loss.log'
    ...
    logger.info(f"Using HAM10000 dataset")
    ...
    # Split the full training dataset into train and validation sets (80-20 split)
    ...
    model_path = f"best_model_ham10k_trainval_loss.pth"
    ...
    if args.testing:
        ...
        if os.path.exists(model_path):
            ...
            test_accuracy, test_loss = evaluate(model, device, test_loader, criterion_cls, logger, mode='Test')
        ...
        for epoch in range(args.epochs):
            ...
            val_accuracy, val_loss = evaluate(model, device, val_loader, criterion_cls, logger, mode='Validation')
